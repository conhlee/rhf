...
const char* __DVDVersion = "<< RVL_SDK - DVD \trelease build: Jul  2 2010 02:03:57 (0x4302_145) >>";
...
static void stateReadingFST(void) {
    ...
    OSAssert(bootInfo->fstSize >= BB2.fstSize, 1093, "DVDChangeDisk(): FST in the new disc is too big.   ");
...
#pragma unused(block)
    ...
    OSSetPeriodicAlarm(&CoverAlarm, (u32)OSGetTick(), OS_MSEC_TO_TICKS(100),
                       CoverAlarmHandler);
}
    ...
    case COMMAND_BSREAD:
        if (block->length <= 0) {
            ...
            block->currTransferSize =
                block->length - block->transferredSize > MAX_TRANSFER_SIZE
                    ? MAX_TRANSFER_SIZE
                    : block->length - block->transferredSize;

            StampCommand(block->command,
                         block->offset + DVD_LOW_OFFSET(block->transferredSize),
                         block->currTransferSize);
    ...
    case COMMAND_READ_UNENCRYPTED:
        if (block->length <= 0) {
            ...
            DVDLowUnencryptedRead(
                (char*)block->addr + block->transferredSize, block->currTransferSize,
                block->offset + DVD_LOW_OFFSET(block->transferredSize),
                cbForStateBusy);
        }
        break;
...
static BOOL IsImmCommandWithResult(u32 command) {
    ...
    for (i = 0; i < ARRAY_LENGTH(ImmCommand); i++) {
...
static void cbForStateBusy(u32 intType) {
    ...
    if (IsDmaCommand(CurrCommand)) {
        executing->transferredSize += intType & (DVD_INTTYPE_BR | DVD_INTTYPE_TC)
                                        ? executing->currTransferSize
                                        : 0;
    }
    ...
    if (intType & DVD_INTTYPE_TC) {
        ...
        if (CurrCommand == COMMAND_STOP_MOTOR) {
            ...
            if (executing->transferredSize != executing->length) {
                ...
                block->state = DVD_STATE_IDLE;
                if (block->callback != NULL) {
                    block->callback(block->transferredSize, block);
                }
