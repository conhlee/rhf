...
const char* __SCVersion =
    "<< RVL_SDK - SC \trelease build: Jul  2 2010 02:08:41 (0x4302_145) >>";
...
static SCNameAndID NameAndIDTbl[SC_ITEM_MAX_PLUS1] = {
...
static s32 SCReloadConfFileAsync(u8* buf, u32 size, SCAsyncCallback callback) {
    ...
    Control.bufferSizes[SC_CONF_FILE_PRODUCT] = ARRAY_LENGTH(OS_SC_PRDINFO);
...
_openFile:
    ...
    OS_SC_PRDINFO[ARRAY_LENGTH(OS_SC_PRDINFO) - 1] = '\0';
...
static SCStatus ParseConfBuf(u8* conf, u32 size) {
    ...
    ItemIDMaxPlus1 = SC_ITEM_MAX_PLUS1;
    ...
    confLutEnd = confEnd - (SC_ITEM_MAX_PLUS1 * sizeof(u16));
...
#pragma unused(block)
    ...
    case NAND_CB_STATE_1:
        if (result == NAND_RESULT_OK &&
            ctrl->fileAttr.permission == NAND_PERM_RWALL) {
            goto _case_5_lbl;
        }
    _case_1_lbl:
